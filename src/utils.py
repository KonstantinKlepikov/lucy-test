...
def save_video(video: torch.Tensor, output_path: str, fps: float = 24.0) -> None:
    ...
    Args:
        video: Video tensor with shape [frames, height, width, channels=3]
               Expected dtype: float32 with values in range [0.0, 1.0] or torch.uint8
...
def get_video_bytes_from_tensor(frames: torch.Tensor, fps: float) -> bytes:
    ...
    Args:
        frames: Video tensor with shape [frames, height, width, channels=3]
                ...
                or torch.uint8 with values in range [0, 255]
                Color format: RGB
...
def generate_edited_video_tensor(
    video: torch.Tensor,
    prompt: str,
    api_key: str,
    fps: float,
...
def generate_edited_video_bytes(video_bytes: bytes, prompt: str, api_key: str) -> bytes:
    ...
    files = {'data': ('video.mp4', video_buffer, 'video/mp4')}

    response = requests.post(
        API_URL, headers={'X-API-KEY': api_key}, data=form_data, files=files
    )
    if response.status_code != 200:
        raise Exception(
            f'Failed to generate edited video: {response.status_code} {response.text}'
        )
